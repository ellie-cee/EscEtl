...
class CustomerCreator(CustomerRecordAwareClient,AddressAwareClient):
    ...
    def run(self):
        ...
        record: MagentoConsolidatedRecord
        shopifyId = None
...
class CompanyCreator(CustomerRecordAwareClient):
    ...
    def loadOrders(self):
        ...
        for recordId in MagentoRecord.recordList("salesOrder"):
            order = MagentoRecord.load(recordId,type="salesOrder",searchable=True)
    ...
    def run(self):
        ...
        for record in self.consolidatedRecordList():
            ...
            record.write()
    def addContactFromSale(self,company:MagentoConsolidatedRecord,order:MagentoConsolidatedRecord):
        contact = {
    ...
    def assignContactToLocations(self,contact,company,roleId):
        if self.getContactOrders(contact)<1:
            if len(company.get("contacts",[]))>=50:
                if isinstance(contact,NetSuiteRecord):
